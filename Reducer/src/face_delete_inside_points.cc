// Created by guinpen98

#include "../include/face.h"

#include <vector>
#include <cmath>
#include <algorithm>

bool ccw(const double x1, const double y1, const double x2, const double y2, const double x3, const double y3){
    double dx1 = x2 - x1;
    double dy1 = y2 - y1;
    double dx2 = x3 - x1;
    double dy2 = y3 - y1;
    return dy1 * dx2 < dx1 * dy2;
}

void Face::DeleteInsidePoints() {
    std::vector<std::pair<double, int>> arg_seq;

    for (int i = 0; i < (int)_points.size(); ++i){
        arg_seq.push_back({std::atan2(_points[i].y,_points[i].x), i});
    }
    std::sort(arg_seq.begin(), arg_seq.end(), [](const std::pair<double, int>& as1, const std::pair<double, int>& as2){return as1.first < as2.first;});
    int head = arg_seq[0].second;
    int pnt = head;
    int start = head;
    double min=_points[arg_seq[0].second].y;
    std::vector<int> next_num((int)_points.size());
    for (int i = 0; i < (int)_points.size(); ++i){
        if(_points[arg_seq[i].second].y < min){
            start=pnt;
            min = _points[arg_seq[i].second].y;
        }
        pnt=next[pnt];
    }
    pnt=start;
    int tmp_cnt=0,cnt=0,next1,next2;
    while(1){
        tmp_cnt=cnt;
        while(1){
            next1=next[pnt];
            next2=next[next1];
            if(ccw(x[pnt],y[pnt],x[next1],y[next1],x[next2],y[next2])==1){
                next[pnt]=next[next1];
                cnt++;
            }
            else{
                pnt=next1;
            }
            if(pnt==start) break;
        }
        if(tmp_cnt==cnt) break;
    }
    pnt=start;

    _points.clear();
    _points = new_points;
}


