...
void FloodFill(const ZMap<ReducerAttribute>& z_map, std::vector<std::vector<int>>& group_map, const int x, const int y){
    ...
    while(!q.empty()){
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(CalculateSimilarity(Vector3d{double(node_x), double(node_y), z_map.z_values[z_map.GetIndex(node_x, node_y)]->z}, Vector3d{double(node_x + points[i].second), double(node_y + points[i].first), z_map.z_values[z_map.GetIndex(node_x + points[i].second, node_y + points[i].first)]->z}) < searcher_threshold){
                continue;            
            }
